...
$_action_labels = array(
	'channel_sync_new'            => __( 'Sync this channel', 'yousync-pro' ),
	'channel_update_all'          => __( "Update this channel's details", 'yousync-pro' ),
	'channel_update_specific'     => __( 'Update specific details of this channel', 'yousync-pro' ),
	'playlists_sync_new'          => __( 'Sync new playlists', 'yousync-pro' ),
	'playlists_update_all'        => __( 'Update all playlist details', 'yousync-pro' ),
	'playlists_update_specific_all' => __( 'Update specific playlist details', 'yousync-pro' ),
	'videos_sync_new'             => __( 'Sync new videos', 'yousync-pro' ),
	'videos_update_all'           => __( 'Update all video details', 'yousync-pro' ),
	'videos_update_specific_all'  => __( 'Update specific video details', 'yousync-pro' ),
);
$_schedule_suffixes = array(
	'once'    => __( 'immediately after saving', 'yousync-pro' ),
	'hourly'  => __( 'every hour', 'yousync-pro' ),
	'daily'   => __( 'every day', 'yousync-pro' ),
	'weekly'  => __( 'every week', 'yousync-pro' ),
	'monthly' => __( 'every month', 'yousync-pro' ),
	/* translators: %d: number of hours */
	'custom'  => sprintf( __( 'every %d hours', 'yousync-pro' ), $custom_schedule ),
);
$rule_action_label    = $action ? ( $_action_labels[ $action ] ?? $action ) : __( 'New rule', 'yousync-pro' );
...
$ys_has_scheduled_sync  = yousync_pro_license()->is_feature_available( 'scheduled_sync' );
$ys_has_metadata_update = yousync_pro_license()->is_feature_available( 'metadata_update' );
$ys_has_conditions      = yousync_pro_license()->is_feature_available( 'conditions' );
$ys_has_field_mapping   = yousync_pro_license()->is_feature_available( 'field_mapping' );
$ys_has_taxonomy        = yousync_pro_license()->is_feature_available( 'taxonomy_terms' );
...
$metadata_options_html  = $resource ? yousync_pro_return_template_part( 'options', $resource . '-metadata' ) : '';

// Field options HTML for conditions (based on resource)
$field_options_tpl = $resource ? yousync_pro_return_template_part( 'options', $resource . '-fields' ) : '';
...
$_max_items_label = $resource === 'video'
	? __( 'Videos per run', 'yousync-pro' )
	: ( $resource === 'playlist'
		? __( 'Playlists per run', 'yousync-pro' )
		: __( 'Items per run', 'yousync-pro' ) );

$_post_type_label = 'playlists_sync_new' === $action
	? __( 'Save synced playlists as post type', 'yousync-pro' )
	: ( 'channel_sync_new' === $action
		? __( 'Save synced channel as post type', 'yousync-pro' )
		: __( 'Save synced videos as post type', 'yousync-pro' )
	);
?>
...
<div class="ys-rule<?php echo $is_syncing ? ' ys-rule--syncing' : ''; ?>" data-rule-index="<?php echo esc_attr( $rule_index ); ?>" data-ch-index="<?php echo esc_attr( (string) $option_channel_idx ); ?>">
	...
	<div class="ys-syncing-overlay" aria-hidden="true">
		<span class="ys-syncing-badge"><?php esc_html_e( 'Syncing...', 'yousync-pro' ); ?><span class="ys-syncing-progress"></span></span>
	</div>
	<?php endif; ?>
	...
	<div class="ys-rule-header" role="button" tabindex="0" aria-expanded="true">
		<div class="ys-rule-heading-wrap">
			...
			<?php if ( ! $enabled ) : ?>
			<p class="ys-rule-disabled-notice"><?php esc_html_e( 'This rule is disabled and won\'t run until re-enabled.', 'yousync-pro' ); ?></p>
			<?php endif; ?>
		</div>
		<div class="ys-rule-actions">
			...
			<?php if ( $has_stat ) : ?>
				<button type="button" class="ys-schedule" aria-label="<?php esc_attr_e( 'Sync schedule info', 'yousync-pro' ); ?>">
					<div class="ys-schedule-tooltip" hidden>
						<?php if ( ! empty( $rule['scheduled_at'] ) && ! $rule_last_synced ) : ?><span class="ys-schedule-item"><span><?php esc_html_e( 'Created:', 'yousync-pro' ); ?></span><span><?php echo esc_html( $rule_stat_created ); ?></span></span><?php endif; ?>
						<?php if ( $rule_last_synced ) : ?><span class="ys-schedule-item"><span><?php esc_html_e( 'Last Sync:', 'yousync-pro' ); ?></span><span><?php echo esc_html( $rule_stat_last_sync ); ?></span></span><?php endif; ?>
						<?php if ( $rule_next_run && 'once' !== $schedule ) : ?><span class="ys-schedule-item"><span><?php esc_html_e( 'Next Sync:', 'yousync-pro' ); ?></span><span><?php echo esc_html( $rule_stat_next_sync ); ?></span></span><?php endif; ?>
					</div>
				</button>
			<?php endif; ?>
	...
	<div class="ys-rule-body">
		...
		<div class="ys-2-columns ys-cols-3-1">
			<div class="ys-form-group">
				<label for="ys-action-<?php echo esc_attr( $rule_index ); ?>">
					<?php esc_html_e( 'Action', 'yousync-pro' ); ?>
					<span class="ys-help-wrap">
						<button type="button" class="ys-help-btn" aria-label="<?php esc_attr_e( 'More info', 'yousync-pro' ); ?>">?</button>
						<span class="ys-help-tooltip" role="tooltip"><?php esc_html_e( 'Choose what to sync from your YouTube channel.', 'yousync-pro' ); ?></span>
					</span>
				</label>
				<select class="ys-select ys-action" id="ys-action-<?php echo esc_attr( $rule_index ); ?>" name="<?php echo esc_attr( $name_prefix ); ?>[<?php echo esc_attr( $rule_index ); ?>][action]" required>
					<option value=""><?php esc_html_e( '— Select action —', 'yousync-pro' ); ?></option>
					<?php $mu = $ys_has_metadata_update ? '' : 'disabled'; ?>
					<optgroup label="<?php esc_attr_e( 'Videos', 'yousync-pro' ); ?>">
						<option data-resource="video" value="videos_sync_new" <?php selected( $action, 'videos_sync_new' ); ?>><?php esc_html_e( 'Sync new videos', 'yousync-pro' ); ?></option>
						<option data-resource="video" <?php echo esc_attr( $mu ); ?> value="videos_update_all" <?php selected( $action, 'videos_update_all' ); ?>><?php esc_html_e( 'Update all video details', 'yousync-pro' ); ?><?php echo $ys_has_metadata_update ? '' : esc_html__( ' (Pro)', 'yousync-pro' ); ?></option>
						<option data-resource="video" <?php echo esc_attr( $mu ); ?> value="videos_update_specific_all" <?php selected( $action, 'videos_update_specific_all' ); ?>><?php esc_html_e( 'Update specific video details', 'yousync-pro' ); ?><?php echo $ys_has_metadata_update ? '' : esc_html__( ' (Pro)', 'yousync-pro' ); ?></option>
					</optgroup>
					<optgroup label="<?php esc_attr_e( 'Playlists', 'yousync-pro' ); ?>">
						<option data-resource="playlist" value="playlists_sync_new" <?php selected( $action, 'playlists_sync_new' ); ?>><?php esc_html_e( 'Sync new playlists', 'yousync-pro' ); ?></option>
						<option data-resource="playlist" <?php echo esc_attr( $mu ); ?> value="playlists_update_all" <?php selected( $action, 'playlists_update_all' ); ?>><?php esc_html_e( 'Update all playlist details', 'yousync-pro' ); ?><?php echo $ys_has_metadata_update ? '' : esc_html__( ' (Pro)', 'yousync-pro' ); ?></option>
						<option data-resource="playlist" <?php echo esc_attr( $mu ); ?> value="playlists_update_specific_all" <?php selected( $action, 'playlists_update_specific_all' ); ?>><?php esc_html_e( 'Update specific playlist details', 'yousync-pro' ); ?><?php echo $ys_has_metadata_update ? '' : esc_html__( ' (Pro)', 'yousync-pro' ); ?></option>
					</optgroup>
					<optgroup label="<?php esc_attr_e( 'Channel', 'yousync-pro' ); ?>">
							<option data-resource="channel" value="channel_sync_new" <?php selected( $action, 'channel_sync_new' ); ?>><?php esc_html_e( 'Sync this channel', 'yousync-pro' ); ?></option>
						<option data-resource="channel" <?php echo esc_attr( $mu ); ?> value="channel_update_all" <?php selected( $action, 'channel_update_all' ); ?>><?php esc_html_e( "Update this channel's details", 'yousync-pro' ); ?><?php echo $ys_has_metadata_update ? '' : esc_html__( ' (Pro)', 'yousync-pro' ); ?></option>
						<option data-resource="channel" <?php echo esc_attr( $mu ); ?> value="channel_update_specific" <?php selected( $action, 'channel_update_specific' ); ?>><?php esc_html_e( 'Update specific details of this channel', 'yousync-pro' ); ?><?php echo $ys_has_metadata_update ? '' : esc_html__( ' (Pro)', 'yousync-pro' ); ?></option>
					</optgroup>
				</select>
			...
			<div class="ys-form-group ys-items-per-run-wrapper<?php echo str_starts_with( $action, 'channel_' ) ? ' ys-hidden' : ''; ?>">
				<label for="ys-max-videos-<?php echo esc_attr( $rule_index ); ?>" class="ys-max-items-label">
					...
					<span class="ys-help-wrap">
						<button type="button" class="ys-help-btn" aria-label="<?php esc_attr_e( 'More info', 'yousync-pro' ); ?>">?</button>
						<span class="ys-help-tooltip" role="tooltip"><?php esc_html_e( 'Maximum number of items to process per run. Set to 0 for unlimited.', 'yousync-pro' ); ?></span>
					</span>
				</label>
				<span class="ys-limit-wrap">
					...
					<span class="ys-unlimited-icon<?php echo 0 === $max_videos ? '' : ' ys-hidden'; ?>" title="<?php esc_attr_e( 'Unlimited — click to set a limit', 'yousync-pro' ); ?>">
		...
		<div class="ys-2-columns <?php echo $show_specific_metadata ? '' : 'ys-hidden'; ?> ys-specific-metadata-wrapper">
			<div class="ys-form-group">
				<label>
					<?php esc_html_e( 'Details to update', 'yousync-pro' ); ?>
					<span class="ys-help-wrap">
						<button type="button" class="ys-help-btn" aria-label="<?php esc_attr_e( 'More info', 'yousync-pro' ); ?>">?</button>
						<span class="ys-help-tooltip" role="tooltip"><?php esc_html_e( 'Select which details to pull from YouTube and save.', 'yousync-pro' ); ?></span>
					</span>
				</label>
				<div class="ys-specific-metadata-rows" data-resource="<?php echo esc_attr( $resource ); ?>"><?php foreach ( $specific_metadata as $saved_value ) :
					...
					<div class="ys-specific-metadata-row">
						<select class="ys-select ys-specific-metadata" name="<?php echo esc_attr( "{$name_prefix}[{$rule_index}][specific_metadata][]" ); ?>">
							<option value=""><?php esc_html_e( '— Select detail —', 'yousync-pro' ); ?></option>
							<?php echo $row_opts; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
						</select>
						<button type="button" class="ys-remove-metadata-field" aria-label="<?php esc_attr_e( 'Remove', 'yousync-pro' ); ?>"></button>
					</div>
					<?php endforeach; ?></div>
				<button type="button" class="ys-add-metadata-field"><?php esc_html_e( 'Add detail', 'yousync-pro' ); ?></button>
			</div>
		</div>

		<div class="ys-2-columns ys-cols-3-1">
			<div class="ys-form-group">
				<label for="ys-sync-schedule-<?php echo esc_attr( $rule_index ); ?>"><?php esc_html_e( 'Sync schedule', 'yousync-pro' ); ?></label>
				<select class="ys-select ys-sync-schedule" id="ys-sync-schedule-<?php echo esc_attr( $rule_index ); ?>" name="<?php echo esc_attr( $name_prefix ); ?>[<?php echo esc_attr( $rule_index ); ?>][schedule]" required>
					<?php yousync_pro_get_template_part( 'options', 'schedule', array( 'selected' => $schedule, 'can_schedule' => $ys_has_scheduled_sync ) ); ?>
				</select>
			</div>
			<div class="ys-form-group ys-custom-schedule-wrapper<?php echo 'custom' !== $schedule ? ' ys-hidden' : ''; ?>">
				<label for="ys-custom-schedule-<?php echo esc_attr( $rule_index ); ?>"><?php esc_html_e( 'Custom (Hours)', 'yousync-pro' ); ?></label>
				<input class="ys-number ys-custom-sync-schedule" id="ys-custom-schedule-<?php echo esc_attr( $rule_index ); ?>" name="<?php echo esc_attr( $name_prefix ); ?>[<?php echo esc_attr( $rule_index ); ?>][custom_schedule]" value="<?php echo esc_attr( $custom_schedule ); ?>" min="1" placeholder="<?php esc_attr_e( 'Eg. 24', 'yousync-pro' ); ?>" type="number">
			</div>
		</div>
		...
		<div class="ys-2-columns ys-post-type-wrapper<?php echo $_show_post_type ? '' : ' ys-hidden'; ?>">
			<div class="ys-form-group">
				<label for="ys-dest-post-type-<?php echo esc_attr( $rule_index ); ?>" class="ys-post-type-label">
					...
					<span class="ys-help-wrap">
						<button type="button" class="ys-help-btn" aria-label="<?php esc_attr_e( 'More info', 'yousync-pro' ); ?>">?</button>
						<span class="ys-help-tooltip" role="tooltip"><?php esc_html_e( 'The WordPress post type that synced items will be created as.', 'yousync-pro' ); ?></span>
					</span>
				</label>
				<select class="ys-select" id="ys-dest-post-type-<?php echo esc_attr( $rule_index ); ?>" name="<?php echo esc_attr( $name_prefix ); ?>[<?php echo esc_attr( $rule_index ); ?>][destination_post_type]" required>
					<option value=""><?php esc_html_e( '— Select post type —', 'yousync-pro' ); ?></option>
				...
				<p class="ys-field-note description"><?php esc_html_e( 'Required — synced items are saved as this post type. If it is left unset (or the post type is later removed), the rule will not sync and the error is logged to the sync history.', 'yousync-pro' ); ?></p>
			</div>
		</div>
		...
		<div class="ys-form-group ys-taxonomy-terms-wrapper<?php echo ! $ys_has_taxonomy ? ' ys-taxonomy-terms-locked' : ''; ?><?php echo $_show_tax_terms ? '' : ' ys-hidden'; ?>">
			<label>
				<?php esc_html_e( 'Assign to taxonomy terms', 'yousync-pro' ); ?>
				<span class="ys-help-wrap">
					<button type="button" class="ys-help-btn" aria-label="<?php esc_attr_e( 'More info', 'yousync-pro' ); ?>">?</button>
					<span class="ys-help-tooltip" role="tooltip"><?php esc_html_e( 'Assign synced items to categories, tags, or other taxonomy terms.', 'yousync-pro' ); ?></span>
				</span>
			</label>
			<div class="ys-taxonomy-terms"><?php foreach ( $destination_taxonomy_terms as $tt_index => $tt ) :
					...
					$_tax_opts = '<option value="">' . esc_html__( '&mdash; Select taxonomy &mdash;', 'yousync-pro' ) . '</option>';
					...
					$_term_opts = '<option value="">' . esc_html__( '&mdash; Select term &mdash;', 'yousync-pro' ) . '</option>';
				...
				<div class="ys-taxonomy-term-row ys-taxonomy-term-row--locked">
					...
					<button type="button" class="ys-remove-taxonomy-term-locked" aria-label="<?php esc_attr_e( 'Remove', 'yousync-pro' ); ?>"></button>
				</div>
				<?php endif;
				endforeach; ?></div>
			<button type="button" class="<?php echo $ys_has_taxonomy ? 'ys-add-taxonomy-term' : 'ys-add-taxonomy-term-locked'; ?>">
				<?php esc_html_e( 'Add taxonomy term', 'yousync-pro' ); ?>
			</button>
		</div>
	...
	$_fm_allowed_sources  = 'playlist' === $resource ? array(
		'playlist_title'       => __( 'Title', 'yousync-pro' ),
		'playlist_description' => __( 'Description', 'yousync-pro' ),
		'playlist_video_count' => __( 'Video Count', 'yousync-pro' ),
		'playlist_thumbnail'   => __( 'Thumbnail URL', 'yousync-pro' ),
	) : array(
		'title'         => __( 'Title', 'yousync-pro' ),
		'description'   => __( 'Description', 'yousync-pro' ),
		'duration'      => __( 'Duration (seconds)', 'yousync-pro' ),
		'view_count'    => __( 'View Count', 'yousync-pro' ),
		'like_count'    => __( 'Like Count', 'yousync-pro' ),
		'published_at'  => __( 'Published Date', 'yousync-pro' ),
		'thumbnail_url' => __( 'Thumbnail URL', 'yousync-pro' ),
		'channel_title' => __( 'Channel Title', 'yousync-pro' ),
	);
	$_fm_btn_label = 'playlist' === $resource
		? __( 'Add playlist detail mapping', 'yousync-pro' )
		: __( 'Add video detail mapping', 'yousync-pro' );
	$_rule_fm_html = '';
	$_rule_fm_idx  = 0;
	foreach ( $_rule_field_mapping as $fm_row ) {
		...
		$_source_opts = '<option value="">' . esc_html__( '— Source —', 'yousync-pro' ) . '</option>';
		...
		if ( $ys_has_field_mapping ) {
			...
			$_rule_fm_html .= '<input type="text" class="ys-text ys-fm-meta-key" name="' . $_fm_name_tgt . '" value="' . esc_attr( $fm_target ) . '" placeholder="' . esc_attr__( 'e.g. _yousync_duration', 'yousync-pro' ) . '">';
			$_rule_fm_html .= '<button type="button" class="ys-remove-field-mapping-row" aria-label="' . esc_attr__( 'Remove', 'yousync-pro' ) . '"></button>';
			$_rule_fm_html .= '</div>';
		} else {
			$_placeholder  = esc_attr__( 'e.g. _yousync_duration', 'yousync-pro' );
			...
			$_rule_fm_html .= '<button type="button" class="ys-remove-field-mapping-row-locked" aria-label="' . esc_attr__( 'Remove', 'yousync-pro' ) . '"></button>';
			$_rule_fm_html .= '</div>';
		}
	...
	$_fm_mapping_labels = array(
		'video'    => __( 'Map video details to post metadata', 'yousync-pro' ),
		'playlist' => __( 'Map playlist details to post metadata', 'yousync-pro' ),
		'channel'  => __( 'Map channel details to post metadata', 'yousync-pro' ),
	);
	$_fm_mapping_label = $_fm_mapping_labels[ $resource ] ?? __( 'Map YouTube details to post metadata', 'yousync-pro' );
	?>
	<div class="ys-form-group ys-field-mapping-wrapper<?php echo ! $ys_has_field_mapping ? ' ys-field-mapping-locked' : ''; ?><?php echo ( $action && strpos( $action, '_update_' ) !== false ) ? ' ys-hidden' : ''; ?>">
		...
		<div class="ys-form-group ys-conditions-wrapper<?php echo $ys_has_conditions ? '' : ' ys-conditions-locked'; ?><?php echo ( ! $action || str_starts_with( $action, 'channel_' ) ) ? ' ys-hidden' : ''; ?>">
			<label class="ys-conditions-label">
				<?php esc_html_e( 'Filter conditions', 'yousync-pro' ); ?>
				<span class="ys-help-wrap">
					<button type="button" class="ys-help-btn" aria-label="<?php esc_attr_e( 'More info', 'yousync-pro' ); ?>">?</button>
					<span class="ys-help-tooltip" role="tooltip"><?php esc_html_e( 'Only sync items that match all of the following conditions.', 'yousync-pro' ); ?></span>
				</span>
			</label>
			...
			if ( ! empty( $conditions ) && is_array( $conditions ) ) :
				foreach ( $conditions as $condition_index => $condition ) :
					...
					if ( $cond_field ) {
						$field_type = yousync_pro_get_condition_field_type( $cond_field );
						if ( $field_type ) {
							$cond_operator_html = yousync_pro_return_template_part(
								'options',
								$field_type . '-operators',
								array( 'operator' => $cond_operator )
							);
							$cond_value_html = yousync_pro_return_template_part(
								'input',
								$field_type,
								array(
					...
					yousync_pro_get_template_part( 'sync-rule', $cond_template, array(
						'rule_index'         => $rule_index,
						'condition_index'    => $condition_index,
						'condition'          => $condition,
			...
			<button type="button" class="<?php echo $ys_has_conditions ? 'ys-add-condition' : 'ys-add-condition-locked'; ?>"><?php esc_html_e( 'Add condition', 'yousync-pro' ); ?></button>
		</div>
	...
	<div class="ys-rule-footer">
		<button type="button" class="ys-remove-rule">
			<?php esc_html_e( 'Remove sync rule', 'yousync-pro' ); ?>
		</button>
	</div>
